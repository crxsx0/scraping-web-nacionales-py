...
async def main():
    ...
    with open('tiendas.json', 'r', encoding='utf-8') as archivo:
        tiendas_construmart = json.load(archivo)
    ...
    async with async_playwright() as p:
        df_construmart = await construmart.scraping_tiendas(p, tiendas_construmart, fecha_extraccion)
        df_csv = df_construmart.to_csv("prueba.csv", index= False)
